...
function makeWord(lettersObject) {
  const stringObj = JSON.stringify(lettersObject);
  const letters = stringObj.match(/[A-Za-z]/g);
  if (letters === 0 || lettersObject === null || letters === null) return '';
  const lengthLetters = Object.values(lettersObject);
  const result = Array(lengthLetters.length);
  letters.forEach((el) => {
    const tempArray = lettersObject[el];
    tempArray.forEach((inEl) => {
      result[inEl] = el;
    });
  });
  return result.join('');
...
function sellTickets(queue) {
  if (queue.length === 0 || queue === null) return true;
  const money = {
    25: 0,
    50: 0,
    100: 0,
  ...
  for (let i = 0; i < queue.length; i += 1) {
    money[queue[i]] += queue[i];
    let change = queue[i] - 25;
    let flag;
    let flag2;
    while (change > 0) {
      flag = change;
      flag2 = change;
      if (money[50] >= 50 && money[50] >= change && change >= 50) {
        money[50] -= 50;
        change -= 50;
        flag2 = change;
      }
      if (money[25] >= 25 && money[25] >= change && change >= 25) {
        money[25] -= 25;
        change -= 25;
        flag2 = change;
      }
